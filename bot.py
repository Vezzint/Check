...
# 2. Курсы крипты от CoinGecko (в RUB и USD)
def get_crypto_rates():
    url = "https://api.coingecko.com/api/v3/simple/price"
    params = {
        "ids": "the-open-network,bitcoin,tether",
...
# Распознавание валюты и суммы из текста
def parse_user_input(text):
    text = text.strip().lower().replace(",", ".")
    
    # Регулярное выражение для поиска числа и валюты
    ...
    cur = "USD"  # По умолчанию для одиночного числа
...
def start_handler(message):
    text = (
        ...
        "• Нажимайте кнопки для мгновенных курсов.\n"
        "• **Фиат в рубли:** `1000$`, `250 eur`, `500 cny`\n"
        "• **Крипта в рубли:** `20 ton`, `0.05 btc`, `300 usdt`\n"
...
def message_handler(message):
    ...
    if not cbr or not crypto:
        bot.send_message(message.chat.id, "⚠️ Сервис временно недоступен. Попробуйте через минуту.")
        return
    ...
    # Обработка пользовательского ввода сумм
    amount, cur = parse_user_input(message.text)
    
    if amount is not None:
        if cur == "RUB":
            # Расчет из рублей во все активы
            ...
            bot.send_message(
                message.chat.id,
                f"{flags[cur]} **{amount:g} {cur}** = `{total:,.2f}` ₽".replace(",", " "),
                parse_mode="Markdown"
            )
            ...
            bot.send_message(
                message.chat.id,
                f"{icons[cur]} **{amount:g} {cur}** = `{total_rub:,.2f}` ₽ (`${total_usd:,.2f}`)".replace(",", " "),
                parse_mode="Markdown"
            )
    else:
        bot.send_message(
            message.chat.id,
            "Не удалось распознать запрос. Примеры:\n`25 ton`\n`0.01 btc`\n`150 usdt`\n`500$`\n`100 000 руб`",
            parse_mode="Markdown"
        )
